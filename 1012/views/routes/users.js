...
router.post("/update", upload.single('file'), function(req,res){
    console.log('...................../update');
    const uid=req.body.uid; //정보를 가져옴... body는 폼(update)에 있어야 사용가능
    const uname=req.body.uname;
    const phone=req.body.phone;
    const address1=req.body.address1;
    const address2=req.body.address2;
    let photo=req.body.photo; 
    if(req.file) photo =req.file.filename; //바뀐 파일이 있다면 바뀐로 바꾸어라
    ...
    db.get().query(sql,[uname,phone,address1,address2,photo, uid],function(err, rows){// 적은 순서는 일치해야함, sql 실행시킴 
        if(err) console.log('error........' , err); //디비 에러 찾을 때 씀 
